...
`;
  ...
  // ─── ENTERPRISE PERMISSIONS MANIFEST ─────────────────────────────────────

  app.get("/api/security/permissions-manifest", authMiddleware, (req, res) => {
    const userContext = req.authContext;
    if (userContext?.userRole !== "System Admin" && userContext?.userRole !== "Admin" && userContext?.userRole !== "Quality Auditor") {
      return res.status(403).json({ error: "Forbidden: Elevated role required to view permissions manifest." });
    }
    res.json({
      _outputClassification: "production-real",
  ...
  // ─── WORKFLOW API ROUTES ─────────────────────────────────────────────────────
  //
  // Workflows track a COA batch through the 5-stage GxP compliance lifecycle:
  ...
  const STAGE_REQUIRED_PERMISSION: Record<WorkflowStage, string> = {
    "Intake": "INGEST_COA",
  ...
  // GET /api/workflows — list all workflows for the authenticated tenant
  app.get("/api/workflows", authMiddleware, async (req, res) => {
    const { tenantId } = req.authContext || {};
    if (!tenantId) return res.status(401).json({ error: "Unauthorized" });
    if (!adminDb) {
      // Local fallback: return empty list
  ...
  // POST /api/workflows — create a new workflow from a COA batch
  app.post("/api/workflows", authMiddleware, requirePermission("CREATE_WORKFLOW"), async (req, res) => {
    const userContext = req.authContext;
  ...
  // POST /api/workflows/:id/transition — advance to the next GxP lifecycle stage
  app.post("/api/workflows/:id/transition", authMiddleware, requirePermission("TRANSITION_WORKFLOW"), async (req, res) => {
    const userContext = req.authContext;
    const { id } = req.params;
    ...
    // Verify the user's role allows them to transition into this specific stage
    const requiredPerm = STAGE_REQUIRED_PERMISSION[toStage as WorkflowStage];
    if (!hasPermission(userContext?.userRole, requiredPerm as any)) {
      return res.status(403).json({
    ...
    if (!adminDb) {
      // Local fallback mode — simulate success
      return res.json({
        success: true,
        fallback: true,
    ...
    try {
      await adminDb.runTransaction(async (transaction) => {
        ...
        const data = doc.data();
        if (data?.tenantId !== (userContext?.tenantId || DEFAULT_TENANT)) {
        ...
        if (targetIndex <= currentIndex) {
          throw new Error(`BAD_REQUEST:Cannot regress workflow. Current stage: '${data?.currentStage}', Requested: '${toStage}'.`);
        }
  ...
  // POST /api/reports/generate — compile + cryptographically seal a compliance/ROI report
  app.post("/api/reports/generate", authMiddleware, requirePermission("GENERATE_REPORT"), async (req, res) => {
    const userContext = req.authContext;
    const token = req.firebaseToken as string;
    const tenantId = userContext?.tenantId || DEFAULT_TENANT;
    ...
    try {
      ...
      // Persist the report metadata to Firestore
      if (adminDb) {
  ...
  // GET /api/reports — list historical compliance reports for the tenant
  app.get("/api/reports", authMiddleware, requirePermission("VIEW_REPORTS"), async (req, res) => {
    const userContext = req.authContext;
    const tenantId = userContext?.tenantId || DEFAULT_TENANT;
